...
def propagate_muons():
    ...
    sector_def.crosssection_defs.weak_def.parametrization = pp.parametrization.weakinteraction.WeakParametrization.CooperSarkarMertsch
    ...
    for mu_energy in muon_energies:
        ...
        for sec in secondarys:
            ...
            if sec.id == pp.particle.Data.Epair:
                ...
                weak_secondary_energy.append(log_sec_energy)
            else:
    ...
    np.savez(
        os.path.join(
            dir_prefix,
            'data_weak_sec_dist_{}_{}_Emin_{}_Emax_{}'.format(
                prop.particle.particle_def.name,
                sector_def.medium.name.lower(),
                E_min_log,
                E_max_log,
    ...
    #statistics:
    num_all = len(brems_secondary_energy) + len(epair_secondary_energy) + len(photo_secondary_energy) + len(ioniz_secondary_energy) + len(mupair_secondary_energy) + len(weak_secondary_energy)
    print(num_all)
    ...
    print("Weak: ", len(weak_secondary_energy), len(weak_secondary_energy)/num_all)


    plot_secondary_spectrum('data_weak_sec_dist_{}_{}_Emin_{}_Emax_{}.npz'.format(
                prop.particle.particle_def.name,
                sector_def.medium.name.lower(),
                E_min_log,
                E_max_log,
...
def plot_secondary_spectrum(data_name):
    ...
    weak_secondary_energy = npzfile['weak']
    ...
    ax_all.hist(
        ...
        log=True,
        bins=100,
        label=['Ionisation', 'Photonuklear', 'Bremsstrahlung', r'$e$ Paarproduktion', r'$\mu$ Paarproduktion', 'Schwache Wechselwirkung', 'Summe']
    )
    ...
    fig_all.savefig("all_weak_{}_stats_{}_Emin_{}_Emax_{}_index_{}.pdf".format(
        medium_name,
        statistics,
        E_min_log,
        E_max_log,
...
if __name__ == "__main__":
    propagate_muons()
    #plot_secondary_spectrum("data_weak_sec_dist_MuMinus_standardrock_Emin_8.0_Emax_8.0.npz")
    #plot_theory_curve()
#
